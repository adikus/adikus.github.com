...
EasyStar.js = function () {
     ...
     *
     **/
    this.findPath = function (startX, startY, endX, endY, fast, callback) {
        ...
        var start = collisionGrid.getTile(startX, startY);
        var end = collisionGrid.getTile(endX, endY);
        if (!start || !end) {
    ...
    this.calculate = function () {
        if (instances.length === 0 || collisionGrid === undefined) {
            return;
        }
        for (iterationsSoFar = 0; iterationsSoFar < iterationsPerCalculation; iterationsSoFar++) {
            ...
            if (instances[0].openList.length === 0) {
                ...
                if(ic.foundPath){
                    ic.callback(ic.foundPath);
                    console.log('Done - empty queue', ic.foundPath);
                }else{
                    ic.callback(null);
                    console.log('Fail', instances[0]);
                }
            ...
            if(instances[0].iterationsSinceLastResult && instances[0].iterationsSinceLastResult > 100000){
                var ic = instances[0];

                ic.isDoneCalculating = true;
                ic.callback(ic.foundPath, true);
                console.log('Done - too long since last', ic.foundPath);
            }

            var searchNode = instances[0].openList.shiftHighestPriorityElement();
            //if(map[searchNode.y+1] && map[searchNode.y+1][searchNode.x+1])
            //	drawPolygon(searchNode.y, searchNode.x, 'rgba(255, 0, 0, 0.3)');

            var tilesToSearch = [];
            if(instances[0].fast)searchNode.list = EasyStar.Node.CLOSED_LIST;

            if (searchNode.y > 0) {
                tilesToSearch.push({
                    instance: instances[0], searchNode: searchNode,
                    x: 0, y: -1, cost: getTileCost(searchNode.x, searchNode.y, searchNode.x, searchNode.y - 1)
                });
            }
            if (searchNode.x < collisionGrid.size - 1) {
                tilesToSearch.push({
                    instance: instances[0], searchNode: searchNode,
                    x: 1, y: 0, cost: getTileCost(searchNode.x, searchNode.y, searchNode.x + 1, searchNode.y)
                });
            }
            if (searchNode.y < collisionGrid.size - 1) {
                tilesToSearch.push({
                    instance: instances[0], searchNode: searchNode,
                    x: 0, y: 1, cost: getTileCost(searchNode.x, searchNode.y, searchNode.x, searchNode.y + 1)
                });
            ...
            if (diagonalsEnabled) {
                if (searchNode.x > 0 && searchNode.y > 0) {
                    ...
                    if (allowCornerCutting ||
                        (isTileWalkable(collisionGrid, searchNode.x, searchNode.y, searchNode.x, searchNode.y - 1) &&
                        isTileWalkable(collisionGrid, searchNode.x, searchNode.y, searchNode.x - 1, searchNode.y))) {
                        ...
                        tilesToSearch.push({
                            ...
                            x: -1,
                            y: -1,
                            cost: getTileCost(searchNode.x, searchNode.y, searchNode.x - 1, searchNode.y - 1)
                        });
                    }
                }
                if (searchNode.x < collisionGrid.size - 1 && searchNode.y < collisionGrid.size - 1) {
                ...
                if (searchNode.x < collisionGrid.size - 1 && searchNode.y > 0) {
                ...
                if (searchNode.x > 0 && searchNode.y < collisionGrid.size - 1) {
        ...
        if(instances[0].iterationsSinceLastResult !== undefined)instances[0].iterationsSinceLastResult += iterationsSoFar;
    };
    ...
    var checkAdjacentNode = function (instance, searchNode, x, y, cost) {
        ...
        if (pointsToAvoid[adjacentCoordinateX + "_" + adjacentCoordinateY] === undefined) {
            ...
            if (instance.endX === adjacentCoordinateX && instance.endY === adjacentCoordinateY) {
                if(!instance.bestCost){
                    instance.initialFind = searchNode.costSoFar + cost;
                    instance.iterationsSinceLastResult = 0;
                }else if(searchNode.costSoFar + cost >= instance.bestCost)return;
                var delta = searchNode.costSoFar + cost - instance.initialFind;
                console.log('Finish', searchNode.costSoFar + cost, instance.initialFind, delta, instance.iterationsSinceLastResult, instance.openList.length);
                if(instance.fast){
                    console.log('Done - fast run');
                    instance.isDoneCalculating = true;
                }
                if(instance.iterationsSinceLastResult > 100000){
                    console.log('Done - too long since last');
                    instance.isDoneCalculating = true;
                }
                instance.iterationsSinceLastResult = 0;
                instance.bestCost = searchNode.costSoFar + cost;
                var path = instance.foundPath = [];
                var pathLen = 0;
                path[pathLen] = {x: adjacentCoordinateX, y: adjacentCoordinateY};
                pathLen++;
                path[pathLen] = {x: searchNode.x, y: searchNode.y};
                pathLen++;
                //console.log(searchNode.x, searchNode.y, searchNode.costSoFar);
                var parent = searchNode.parent;
                while (parent != null) {
                    //console.log(parent.x, parent.y, parent.costSoFar);
                ...
                var ic = instance;
                var ip = path;
                ic.callback(ip, instance.isDoneCalculating);
                return
            }

            if (isTileWalkable(collisionGrid, searchNode.x, searchNode.y, adjacentCoordinateX, adjacentCoordinateY)) {
                ...
                if (node.list === undefined) {
                    ...
                    //game.isoProjector.drawOverlay(game.map.getTile(adjacentCoordinateX,adjacentCoordinateY), game.map.pathOverlay);
    ...
    // Helpers
    var isTileWalkable = function (collisionGrid, x1, y1, x2, y2) {
        ...
        //
        //return false;
        var p1 = collisionGrid.getTile(x1, y1);
        var p2 = collisionGrid.getTile(x2, y2);

        if(p2.top <= 0)return false;

        var d = Math.sqrt(Math.pow(x1 - x2, 2) + Math.pow(y1 - y2, 2));
        var h1 = (p1.top + p1.bottom)/2;
        var h2 = (p2.top + p2.bottom)/2;
        var h = Math.abs(h1 - h2);
        //if(p2.bottom == 83)console.log(d, h1, h2, h, p1, p2);

        return h / d < 1;
    };

    var getTileCost = function (x1, y1, x2, y2) {
        //console.log(collisionGrid[y][x]);
        //return pointsToCost[x + '_' + y] || costMap[collisionGrid[y][x]]
        var p1 = collisionGrid.getTile(x1, y1);
        var p2 = collisionGrid.getTile(x2, y2);
        var d_cost = Math.sqrt(Math.pow(x1 - x2, 2) + Math.pow(y1 - y2, 2));
        var z_cost = Math.sqrt(Math.pow((p1.top - p1.bottom) / 2 - (p2.top - p2.bottom) / 2, 2));
        //console.log(x1, y1, x2, y2, cost);
        //return {d: d_cost, z: z_cost, total: d_cost + z_cost*Z_COST};
        var slopeCost = p2.top - p2.bottom;

        return d_cost + z_cost + slopeCost * Z_COST;
    };

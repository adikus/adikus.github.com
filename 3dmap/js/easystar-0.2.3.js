...
EasyStar.js = function() {
	...
	**/
	this.calculate = function() {
		...
		for (iterationsSoFar = 0; iterationsSoFar < iterationsPerCalculation; iterationsSoFar++) {
			...
			if (instances[0].openList.length === 0) {
				...
				for(var i in instances[0].nodeHash){
					console.log(instances[0].nodeHash[i]);
					var x = instances[0].nodeHash[i].x;
					var y = instances[0].nodeHash[i].y;
					if(map[y+1] && map[y+1][x+1]){
						//drawPolygon(y, x, 'rgba('+255+',0,0,1)');
						window.renderContext.strokeText(Math.round(instances[0].nodeHash[i].costSoFar/100).toString(), map[y][x][1], map[y][x][0] + 10);
					}
				}
			...
			if (diagonalsEnabled) {
				if (searchNode.x > 0 && searchNode.y > 0) {
					...
					if (allowCornerCutting ||
						...
						tilesToSearch.push({ instance: instances[0], searchNode: searchNode, 
							x: -1, y: -1, cost: getTileCost(searchNode.x, searchNode.y, searchNode.x-1, searchNode.y-1)});
					}
				}
	...
	var checkAdjacentNode = function(instance, searchNode, x, y, cost) {
		...
		if (pointsToAvoid[adjacentCoordinateX + "_" + adjacentCoordinateY] === undefined) {
			...
			if (instance.endX === adjacentCoordinateX && instance.endY === adjacentCoordinateY) {
				console.log('Finish', searchNode.costSoFar + cost, instance, instance.isDoneCalculating);
	...
	var isTileWalkable = function(collisionGrid, x1, y1, x2, y2) {
        ...
        //
		//return false;
		var p1 = collisionGrid[y1][x1];
		var p2 = collisionGrid[y2][x2];
		var d = Math.sqrt(Math.pow(p1[0] - p2[0],2) + Math.pow(p1[1] - p2[1],2));
		var h = Math.abs(p1[2] - p2[2]);
	...
	var getTileCost = function(x1, y1, x2, y2) {
		...
		var p1 = collisionGrid[y1][x1];
		var p2 = collisionGrid[y2][x2];
		return Math.sqrt(Math.pow(p1[0] - p2[0],2) + Math.pow(p1[1] - p2[1],2) + zCost*Math.pow(p1[2] - p2[2],2));
	};
